...
namespace Natasha.CSharp.HotExecutor.Core
{
    public class VSCSProcessor
    {
        ...
        public VSCSProcessor()
        {
            ...
            {
                FileName = "dotnet.exe",
                WorkingDirectory = VSCSProjectInfoHelper.SlnPath,
        ...
        {
            _outpuNewAppFolder = Path.Combine(VSCSProjectInfoHelper.BinPath, "T" + Guid.NewGuid().ToString("N"));
            _outputNewExeFile = Path.Combine(_outpuNewAppFolder, VSCSProjectInfoHelper.ExecuteName);
            _builder.Arguments = $"build \"{VSCSProjectInfoHelper.MainCsprojPath}\" -c {(HEProxy.IsRelease?"Release":"Debug")} --nologo -o \"{_outpuNewAppFolder}\"";
        }
        ...
        public ValueTask<bool> Run()
        {
            try
            {
                ...
                var process = new Process()
                {
                    StartInfo = new()
                    {
                        FileName = VSCSProjectInfoHelper.ExecuteName,
...
#if DEBUG
                HEProxy.ShowMessage($"执行: {Path.Combine(_outpuNewAppFolder, VSCSProjectInfoHelper.ExecuteName)}");
#endif
                process.Start();
                // 等待一小段时间，让进程有机会启动
