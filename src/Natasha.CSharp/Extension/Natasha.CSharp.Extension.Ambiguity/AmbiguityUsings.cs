...
namespace Natasha.CSharp.Extension.Ambiguity
{
    ...
    {
        internal static readonly Func<AssemblyCSharpBuilder, CSharpCompilation, bool, CSharpCompilation> SemanticAction;
        static AmbiguityUsings()
        {
            SemanticAction = (builder, compilation, ignoreAccessibility) =>
            {
                ...
                {
                    compilation = HandlerCS0104(tree, compilation, builder.LoadContext!.UsingRecorder, ignoreAccessibility);
                }
                return compilation;
                        ...
                        for (int i = 0; i < info.CandidateSymbols.Length - 1; i++)
                        {
                            ...
                            if (!usings.HasUsing(spaceName))
                            {
                                ...
                                var node = (from usingDeclaration in root.Usings
                                            where usingDeclaration.Name != null && usingDeclaration.Name.ToString() == spaceName
